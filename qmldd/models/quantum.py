...
class VariationalQuantumClassifier(BaseModel):
    ...
    def __init__(
        ...
        self.angle_scaler = MinMaxScaler(feature_range=(0, np.pi), clip=True)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        threshold = getattr(self, "threshold", 0.0)
        return 1 / (1 + np.exp(-(raw - threshold)))
...
class QuantumNeuralNetwork(BaseModel):
    ...
    Features an AngleEmbedding feature map, StronglyEntanglingLayers ansatz
    (with 3 Euler rotations per qubit per layer and circular CNOT entanglement),
    followed by multi-wire Pauli-Z expectation readout and a trainable
    classical neural layer (w^T * z + b) with logistic activation.
    Jointly trained using PennyLane parameter-shift gradients and Adam optimizer.
    """
    ...
    def _forward(self, x, q_weights, nn_weights, nn_bias):
        z_expectations = pnp.stack(self.circuit(x, q_weights))
        return pnp.dot(nn_weights, z_expectations) + nn_bias
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray) -> "QuantumNeuralNetwork":
        ...
        class_weight = len(y_train) / (2.0 * np.maximum(class_counts, 1))
        sample_weight = pnp.array(class_weight[y_train.astype(int)])

        q_weight_shape = qml.StronglyEntanglingLayers.shape(n_layers=self.n_layers, n_wires=self.n_qubits)
        self.weights = pnp.array(rng.uniform(0, 2 * np.pi, size=q_weight_shape), requires_grad=True)
        self.nn_weights = pnp.array(rng.uniform(-0.5, 0.5, size=self.n_qubits), requires_grad=True)
        self.nn_bias = pnp.array(0.0, requires_grad=True)

        opt = qml.AdamOptimizer(stepsize=self.lr)
        n_samples = len(X_scaled)

        def cost(qw, nw, nb, X_batch, y_batch, w_batch):
            logits = pnp.stack([self._forward(x, qw, nw, nb) for x in X_batch])
            probs = 1 / (1 + pnp.exp(-logits))
            return pnp.mean(w_batch * (probs - y_batch) ** 2)
        ...
        for _ in range(self.epochs):
            ...
            for start_idx in range(0, n_samples, self.batch_size):
                idx = perm[start_idx : start_idx + self.batch_size]
                X_batch, y_batch, w_batch = X_scaled[idx], y_train[idx], sample_weight[idx]
                (self.weights, self.nn_weights, self.nn_bias), epoch_loss = opt.step_and_cost(
                    lambda qw, nw, nb: cost(qw, nw, nb, X_batch, y_batch, w_batch),
                    self.weights,
                    self.nn_weights,
                    self.nn_bias,
                )
        ...
        train_scores = self._decision_values(X_train)
        self.threshold = _best_threshold(train_scores, y_train.astype(int))
        return self

    def _decision_values(self, X: np.ndarray) -> np.ndarray:
        X_scaled = self.angle_scaler.transform(X)
        return np.array([
            float(self._forward(x, self.weights, self.nn_weights, self.nn_bias))
            for x in X_scaled
        ])
    ...
    def metadata(self) -> dict[str, Any]:
        q_params = int(np.size(self.weights)) if self.weights is not None else 0
        nn_params = int(np.size(self.nn_weights)) + 1 if self.nn_weights is not None else 0
        return {
            "n_qubits": self.n_qubits,
            "n_layers": self.n_layers,
            "circuit_depth": self.n_layers * 2,
            "n_params": q_params + nn_params,
            "quantum_params": q_params,
            "classical_params": nn_params,
            "final_train_loss": self.loss_history[-1] if self.loss_history else None,
            "training_time_seconds": self.training_time_seconds,
            "decision_threshold": getattr(self, "threshold", 0.0),
        }
